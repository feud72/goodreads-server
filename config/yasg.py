...
응답
 - 성공시 200 {"message", "token", "id"}
 - 실패시 400 {"detail"} 또는 {"non_field_errors"} (JSON 형태로 반환)

#### /accounts/signup/ : 회원가입
필수 인자 : email, password1, password2
응답 
 - 성공시 201 {"message", "email"}
 - 실패시 400 {"detail"} 또는 {"non_field_errors"} (JSON 형태로 반환)
...
필수 인자 : isbn
응답
 - 성공시 201 {"title", "author", "publisher", "pub_year", "isbn", "description", "bookImage"} 
 - 실패시 400 {"isbn"} (JSON 형태로 반환)
